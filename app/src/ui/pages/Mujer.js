...
class Mujer extends React.Component {    
    ...
    componentDidMount(){
        ...
        this.props.pedirProductos("mujeres")
    }
    ...
    render(){
        ...
        return (
            <Fragment>
                <h2 id="titulo-mujer">Mujeres</h2>
                <ButtonGroup className="grupoBotones" fullWidth aria-label="full width outlined button group">

                    {marcas.map((marca,i)=>
                        
                        <Button key={i} variant="outlined" size="medium" color="primary" onClick={pedirProductos.bind(null,`mujeres?marca=${marca}`)}>{marca}
                        </Button>
                    )}
                </ButtonGroup>
                <ButtonGroup className="grupoBotones" fullWidth aria-label="full width outlined button group">
                    ...
                    {categorias.map((categoria,i)=>
                        ...
                        <Button key={i} variant="outlined" size="medium" color="primary" onClick={pedirProductos.bind(null,`mujeres?categoria=${categoria}`)}>{categoria}
                        </Button>
                    )}
                </ButtonGroup>
                <div>
